...
def compile_with_xc8_wrapper_direct(optimization_level="2", xc8_version="3.00", separate_compilation=True):
    ...
    try:        
        ...
        if separate_compilation:
            return compile_separate_with_xc8_wrapper(source_files, output_file, optimization_level, xc8_version)
        else:
            return compile_monolithic_with_xc8_wrapper(source_files, output_file, optimization_level, xc8_version)
...
def compile_separate_with_xc8_wrapper(source_files, output_file, optimization_level, xc8_version):
...
def compile_monolithic_with_xc8_wrapper(source_files, output_file, optimization_level, xc8_version):
...
def main():
    parser = argparse.ArgumentParser(description="PIC16F876A compilation with xc8-wrapper")
    parser.add_argument("--optimization", "-O", default="2",
                       help="Optimization level (0-3)")
    parser.add_argument("--xc8-version", default="3.00",
                       help="XC8 version to use")
    ...
    compilation_mode.add_argument("--separate", action="store_true", default=True,
                                help="Use separate compilation (default)")
    compilation_mode.add_argument("--monolithic", action="store_true", 
                                help="Use monolithic compilation")
    ...
    separate_mode = not args.monolithic  # If --monolithic is specified, separate_mode = False
    
    success = compile_with_xc8_wrapper_direct(args.optimization, args.xc8_version, separate_mode)
